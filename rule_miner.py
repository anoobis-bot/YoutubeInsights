...
class RuleMiner(object):
    ...
    def get_support(self, data, itemset):
        ...
        # function.
        itemset_present = data[itemset].all(axis = 1)
        support = itemset_present.sum()
        return support
    ...
    def get_confidence(self, data, rule):
        ...
        # class.
        rule_concat = self.merge_itemsets(rule)
        support_xy = self.get_support(data, rule_concat[0])
    ...
    def get_association_rules(self, data):
        ...
        for rule in rules:
            ...
            for i in range(1, n_rules):
                confidence = self.get_confidence(data, rule[i])
                if confidence >= self.confidence_t:
                    association_rules.append(rule[i])
