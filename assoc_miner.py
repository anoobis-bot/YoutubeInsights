...
class RuleMiner(object):
    ...
    def get_confidence(self, data, rule):
        ...
        # class.
        numerator = self.get_support(data, list(set(rule[0]) | set(rule[1])))
        denominator = self.get_support(data, rule[0])
    ...
    def get_association_rules(self, data):
        ...
        for rule_group in rules:
            ...
            for rule in rule_group:
                conf_lvl = self.get_confidence(data, rule)
                if conf_lvl >= self.confidence_t:
